  ...
  <v-app>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          ...
          <v-btn
            ...
            v-if="getAuthenticatedUser.role == 'Shift Manager'"
          >
          ...
          <v-btn
            ...
            dark
            class="mb-2"
            v-if="getAuthenticatedUser.role == 'Foremen'"
          >
            Add Voyage
          </v-btn>
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-btn depressed rounded color="#66BB6A" @click="openShowReport(item)">
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      this.deleteVesselAction({ id: this.editedItem.id })
        .then(() => {
    ...
    openHistory(item) {
      ...
      this.setActionHistoryAction({ vessel_id: this.editedItem.id })
        .then(() => {
          this.historyTable = this.getActionHistory;
          this.setModuleShowToFalseAction();
          this.dialog4 = true;
        })
        .catch((error) => {
    ...
    fillTheCraneStats(voyage) {
      ...
      this.getCranes.map((c) => {
        ...
        console.log("crane voy :", craneVoyage);
        if (craneVoyage.length > 0) {
          console.log("c ", c);
