  ...
  methods: {
    ...
    checkIfNumberFullAndFromAndToEmpty() {
      ...
      let lgbohc_all_toIsEmpty =
        this.secondFormData.lgbohc_all_to == "" ||
        this.secondFormData.lgbohc_all_to == null;
      console.log("lgbohc_all_fromIsEmpty", lgbohc_all_fromIsEmpty);
      console.log("lgbohc_all_toIsEmpty", lgbohc_all_toIsEmpty);
      ...
      if (
        ...
        this.warninText =
          "When 'FROM' datetime is inserted number of gear boxes and hatch covers and In Bay must not be zero !";
        this.errorDialog = true;
        return true;
      }
      return false;
    ...
    checkIfNumberFullAndFromAndToEmpty2() {
      ...
      if (lss_all_fromIsEmpty && !lss_all_toIsEmpty) {
        ...
        (lss_all_num_ssIsEmpty || lss_all_ib_lnwIsEmpty || lss_all_ibIsEmpty) &&
        !lss_all_fromIsEmpty
      ) {
        this.warninText =
          "When 'FROM' datetime is inserted number of ship's spreaders and In Bay is Loaded by NS or WIRES must be inserted too !";
        this.errorDialog = true;
        return true;
      }
      return false;
